...
#
#Data is from the United States Mortality Database (https://usa.mortality.org/) and Human Mortality Database (https://mortality.org/)  
##########
...
#US life table data (from HMD)
us_lt_b <- read_table("C:/.../bltper_1x1.txt", skip = 2)
...
brass<-brass %>% 
  left_join(brass_standard, by = c("Age")) %>% 
  filter(Age %in% (1:100))
